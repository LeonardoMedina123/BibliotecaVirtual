...
<head>
    ...
    <title>Panel Admin - Editar Libro</title>
...
<body class="bg-gray-100 font-sans antialiased">
    ...
    <main class="max-w-2xl mx-auto px-6 py-8">
        <h1 class="text-3xl font-bold text-slate-900 mb-6">Editar Libro</h1>
        ...
        <form action="{{ route('admin.libros.update', $libro->id_libros) }}" method="POST" class="bg-white rounded-lg shadow p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="url" name="portada_url" id="portada_url" value="{{ old('portada_url', $libro->portada_url) }}" 
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            ...
            <div class="mb-6">
                ...
                <select name="Categorias_id_categorias" id="Categorias_id_categorias" 
                    ...
                    @foreach($categorias as $categoria)
                        <option value="{{ $categoria->id_categorias }}" {{ old('Categorias_id_categorias', $libro->Categorias_id_categorias) == $categoria->id_categorias ? 'selected' : '' }}>
                            {{ $categoria->nombre }}
                        </option>
                    @endforeach
                </select>
